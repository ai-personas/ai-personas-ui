...
export default function InferenceEvidence({ value, open }: { value: unknown; open: (id: string) => void }) {
  ...
  return <section class="reader-section" aria-label="Inference evidence"><h3>Inference evidence</h3>
    ...
    {facts.memory && <section><h4>Learning branches shown</h4><p>{number(facts.memory.offered?.length)} branch descriptions were included. Descriptions are separate from full selected prompt fragments.</p><References value={facts.memory.offered} open={open}/></section>}
