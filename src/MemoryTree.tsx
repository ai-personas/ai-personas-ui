...
export default function MemoryTree({ owner, open }: { owner: string; open: (id: string) => void }) {
  ...
  return <section class="memory-tree" aria-label="Learning tree" aria-busy={!activity && loading}>
    ...
    {activity ? <Suspense fallback={<p>Loading learning activity…</p>}><Activity owner={owner} open={open}/></Suspense> : <>
      <form class="memory-search" onSubmit={e => {e.preventDefault(); setQuery(search.trim()); setBranch(''); setPages([0]);}}><label>Find learning<input type="search" value={search} maxLength={512} aria-describedby={`memory-search-help-${owner}`} onInput={e => setSearch(e.currentTarget.value)}/></label><button>Search learning</button>{query && <button type="button" class="secondary" onClick={() => navigate('')}>Clear search</button>}</form>
