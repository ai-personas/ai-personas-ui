...
function RecordCard({ record, open, artifact, act }: { record: Entity; act: Act } & Links) {
  ...
  return <article class={`work-record${inputRequestCount(r) ? ' needs-input' : ''}`} data-kind={r.kind}>
    ...
    {r.kind === 'request' && <><NamedField name="Evidence required" value={d.evidence_required}/><p class="record-caveat">{d.visibility === 'work' && 'You and other participants can answer this shared question. '}A response does not resolve the question until its owner records a disposition.</p></>}
