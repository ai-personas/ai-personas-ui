...
function Reference({ id, caption, open }: { id: unknown; caption: string; open: Open }) {
  return isRecordID(id) ? <button class="reference-link" onClick={() => open(id)}>{caption} <code>{id.slice(0, 8)}</code><span aria-hidden="true"> ↗</span></button> : null;
}
function TextValue({ value }: { value: unknown }) {
  const lines = typeof value === 'string' ? [value] : strings(value);
  return <>{lines.filter(Boolean).map((line, i) => <p class="record-prose" key={i}>{line}</p>)}</>;
}
...
function RecordCard({ record: r, open, artifact }: { record: Entity } & Links) {
  ...
  const body = text(d.summary) || text(d.description) || text(d.agenda) || text(d.text) || text(d.content) || text(d.purpose) || text(d.note);
  return <article class="work-record" data-kind={r.kind}>
    ...
    {r.kind === 'submission' && <>
      <References ids={d.artifacts} caption="Open artifact" open={artifact}/><References ids={d.documents} caption="Open document" open={open}/>
...
function References({ ids, caption, open }: { ids: unknown; caption: string; open: Open }) {
  return <div class="record-references">{recordIDs(ids).slice(0, 12).map(id => <Reference key={id} id={id} caption={caption} open={open}/>)}
    {recordIDs(ids).length > 12 && <p class="micro">More references are available in the exact record.</p>}</div>;
}
function RecordsSection({ title, description, kinds, work, empty, open, artifact }: {
  title: string; description?: string; kinds: string; work: string; empty: string;
} & Links) {
  ...
  return <section class="workspace-section" aria-label={title} aria-busy={loading}>
    ...
    <div class="work-records">{page?.items.map(record => <RecordCard key={record.id} record={record} open={open} artifact={artifact}/>)}</div>
...
function RosterPerson({ id, open }: { id: string; open: Open }) {
  const { value: r, error } = useResource<Entity>('/records/' + id, e => e.entity === id);
  return <article class="roster-person"><span class="persona-initial" aria-hidden="true">{r ? label(r).slice(0, 1) : '·'}</span><div>
    <button class="record-title" onClick={() => open(id)}>{r ? label(r) : 'Selected persona ' + id.slice(0, 8)}</button>
    <p>{r ? text(data(r).character, 'Character not authored.') : error ? 'Details unavailable.' : 'Loading identity…'}</p>
    <small>Selected participant · no profession inferred</small>
  </div></article>;
}
function Roster({ work, open }: { work: Entity; open: Open }) {
  const [page, setPage] = useState(0), ids = recordIDs(fields(work.data).personas), size = 12;
  return <section class="workspace-section"><h2>Continuing individuals</h2><p class="section-description">The work's recorded roster does not establish membership consent, accepted responsibilities, or contribution quality.</p>
    {ids.length ? <div class="roster-grid">{ids.slice(page * size, (page + 1) * size).map(id => <RosterPerson key={id} id={id} open={open}/>)}</div>
      : <Empty title="No participants recorded">Create or select personas explicitly. The UI never supplies a profession roster.</Empty>}
...
export default function Workspace({ id, open, artifact, back, act }: { id: string; back: () => void; act: Act } & Links) {
  ...
  const section = (title: string, kinds: string, empty: string, description?: string) =>
    <RecordsSection key={tab + ':' + kinds + ':' + id} title={title} kinds={kinds} work={id} empty={empty} description={description} open={open} artifact={artifact}/>;
  return <div class="workspace">
    ...
    <section id="workspace-panel" role="tabpanel" aria-labelledby={`workspace-tab-${WORK_TABS.indexOf(tab)}`} tabIndex={0}>
      ...
      {tab === 'Perspectives' && <>
        ...
        {section('Individual agendas', 'perspective', 'No work-scoped shared perspectives are available. Character is not reverse-engineered into a priority list.')}
