...
export default function Detail({ id, open, artifact, close, act }: { id: string; open: (id: string) => void; artifact: (id: string) => void; close: () => void; act: Act }) {
  ...
  const d = r ? data(r) : {};
  const tabs = r?.kind === 'work' ? ['Activity', 'Submissions', 'Assessments', 'Requests'] : r?.kind === 'persona' ? ['Work', 'Learning', 'Tools', 'Messages', 'History'] : r?.kind === 'environment' ? ['Work', 'Learning', 'Tools', 'Messages'] : r?.kind === 'run' ? ['Actions', 'Model calls'] : r?.kind === 'request' ? ['Responses'] : [];
...
function Respond({ id, act }: { id: string; act: Act }) { const [files, setFiles] = useState<{ id: string; name: string }[]>([]), [busy, setBusy] = useState(false), [sent, setSent] = useState(''); return <form onSubmit={async e => { e.preventDefault(); if (busy) return; const form = e.currentTarget; setBusy(true); try { await act('request.respond', { request: id, text: String(new FormData(form).get('text')), artifacts: files.map(f => f.id), basis: String(new FormData(form).get('basis')) }); form.reset(); setFiles([]); setSent('Response delivered to the owner; disposition is not established.'); } catch { /* Parent displays the failure. */ } finally { setBusy(false); } }}><label>Response type<select name="basis"><option value="answer">Answer or decision</option><option value="assumption">Assumed scenario</option><option value="evidence">Evidence or observation</option><option value="challenge">Challenge or alternative</option></select></label><label>Your response<textarea name="text" rows={4} required placeholder="Facts, observations, a decision, or measurements…"/></label><Suspense fallback={<p>Loading attachment control…</p>}><Upload onFile={(ref, name) => setFiles(fs => [...fs, { id: ref, name }])}/></Suspense>{files.map(f => <p key={f.id}>{f.name} <button type="button" class="quiet" onClick={() => setFiles(files.filter(x => x.id !== f.id))}>Remove</button></p>)}<button disabled={busy}>{busy ? 'Sending…' : 'Send response'}</button>{sent && <p role="status">{sent}</p>}</form>; }
