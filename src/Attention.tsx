...
export function InputBadge({ record }: { record: Entity }) {
  ...
  if (record.kind === 'request' && fields(record.data).status === 'open' && fields(record.data).visibility === 'work' && fields(record.data).requires_human !== true) return <span class="shared-question-badge">Open to peers · your reply is optional</span>;
...
export function InputNotice({ record, open }: { record: Entity; open: (id: string) => void }) {
  const shared = record.kind === 'request' && fields(record.data).status === 'open' && fields(record.data).visibility === 'work' && fields(record.data).requires_human !== true;
