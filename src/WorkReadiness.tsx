    ...
    {value && <>
      ...
      <div class="work-records">{people.map(p => {
        ...
        return <article class="work-record" key={text(p.run)}>
          ...
          {typeof counts.match === 'number' && <p>Latest selector assessment: {String(counts.match)} match · {String(counts.no_match)} no match · {String(counts.unknown)} unknown. Only a match recommends a connection; the persona can also select fragments directly.</p>}
