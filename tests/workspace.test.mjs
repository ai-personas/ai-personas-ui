...
test('only unanswered requests need user input, not waiting runs or answered requests', () => {
  assert.equal(inputRequestCount(record('request', { status: 'open', audience: 'user', requires_human: true })), 1);
  ...
  assert.equal(inputRequestCount(record('request', { status: 'open', audience: 'user', visibility: 'work', requires_human: false })), 0);
