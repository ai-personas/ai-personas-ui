...
assert.match(portal, /WORKSPACE SNAPSHOT · SIGNATURE CHECKED/);
assert.match(portal, /ArtifactBundle lifecycle is unknown/);
assert.doesNotMatch(portal, /KERNEL-SIGNED · VERIFIED|last accepted revision/);
assert.match(portal, /BYTES CHECK FAILED\/REFUSED/);
assert.match(portal, /BYTES NOT CHECKED/);
assert.match(portal, /Hash-checked prior\/current text diff/);
assert.doesNotMatch(portal, /Verified prior\/current text diff/);
assert.match(portal, /bundle-lifecycle-unknown/);
...
assert.match(index, /discovery\.js\?v=20260716-public-run-autotrack-v3/);
